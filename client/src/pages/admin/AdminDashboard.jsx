...
// Professional Admin Dashboard — ReadNOVA
// ============================================================
...
// ═════════════════════════════════════════════════════════════
const fmt = (n) =>
  new Intl.NumberFormat("en-BD").format(Math.round(n ?? 0));
...
const StatCard = ({ title, value, Icon, sub, color, bg }) => (
  <div style={{ background:"#fff", borderRadius:14, padding:"20px 22px",
    ...
    <p style={{ fontSize:11, color:"#9ca3af", fontWeight:600, textTransform:"uppercase",
...
// ── Bar Chart (pure SVG — beautiful with hover) ───────────────
const BarChart = ({ data }) => {
  const [hovered, setHovered] = useState(null);

  // show all 12 months always
    ...
    <div style={{ position:"relative" }}>
      ...
      <svg viewBox={`0 0 ${W} ${H + 32}`} width="100%"
        ...
        <defs>
          ...
          <filter id="bShadow">
            <feDropShadow dx="0" dy="3" stdDeviation="3"
                          floodColor="#1a6b7c" floodOpacity="0.2"/>
          </filter>
        </defs>
          ...
          return (
            <g key={f}>
              <line x1={PAD_L} y1={gy} x2={W - PAD_R} y2={gy}
                    stroke={f === 0 ? "#cbd5e1" : "#f1f5f9"}
                    strokeWidth={f === 0 ? 1.5 : 1}
                    strokeDasharray={f > 0 ? "4 3" : "none"}/>
              <text x={PAD_L - 8} y={gy + 4} textAnchor="end"
        ...
        {fullData.map((d, i) => {
          ...
          const x   = cx - barW / 2;
          const bh  = d.value > 0 ? Math.max(6, (d.value / max) * H) : 3;
          const y   = H - bh;
          const isH = hovered === i;
          const isCur = d.label === curMon;
          ...
          return (
            <g key={i}
              ...
              {/* hover column bg */}
              {isH && (
                <rect x={cx - slotW/2 + 2} y={4} width={slotW - 4} height={H - 4}
              ...
              {isH && d.value > 0 && (
                <g>
                  <rect x={cx - 32} y={y - 26} width={64} height={18}
                        fill="#1a1a1a" rx={4} opacity="0.88"/>
                  <text x={cx} y={y - 13} textAnchor="middle"
              ...
              {/* value label (not hovered) */}
              {!isH && d.value > 0 && (
                <text x={cx} y={y - 5} textAnchor="middle"
                      fontSize="7.5" fontWeight="700" fill="#1a6b7c" opacity="0.75">
              ...
              <text x={cx} y={H + 17} textAnchor="middle"
                    fontSize="9" fill={isH ? "#1a6b7c" : isCur ? "#1a6b7c" : "#94a3b8"}
              ...
              {/* current month dot */}
              {isCur && (
                <circle cx={cx} cy={H + 26} r={2.5} fill="#1a6b7c"/>
              )}
            </g>
          );
        })}
        ...
        {/* baseline */}
        <line x1={PAD_L} y1={H} x2={W - PAD_R} y2={H}
...
const ActionCard = ({ Icon, label, to, onClick, color }) => {
  const inner = (
    <div style={{ background:"#fff", borderRadius:14, padding:"18px 14px",
...
export default function AdminDashboard() {
  ...
  const downloadReport = () => {
    const rows = [
      ...
      ["Weekly Sales",    `BDT ${weeklySales}`],
      ["Avg Order Value", `BDT ${avgOrder}`],
    ...
    <div style={{ fontFamily:"'Plus Jakarta Sans',sans-serif",
        ...
        <div style={{ position:"relative", zIndex:1, display:"flex",
          ...
          <div style={{ display:"flex", gap:10 }}>
            ...
            <button onClick={downloadReport}
              style={{ display:"flex", alignItems:"center", gap:7,
                       padding:"10px 18px", background:"#f5a623",
      ...
      <div style={{ padding:"28px 32px" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",
          ...
          <StatCard title="Pending"       value={fmt(pendingOrders)}
            Icon={HiOutlineClock} color="#ca8a04" bg="#fefce8"
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 300px",
          ...
          <div style={{ background:"#fff", borderRadius:14, padding:"22px 24px",
            ...
            <h3 style={{ fontFamily:"Georgia,serif", fontSize:15, fontWeight:700,
                          color:"#1a1a1a", marginBottom:16, display:"flex",
          ...
          <div style={{ background:"#fff", borderRadius:14, padding:"22px 24px",
            ...
            <div style={{ display:"flex", justifyContent:"space-between",
              ...
              <Link to="/admin/orders"
                style={{ fontSize:12, color:"#1a6b7c", fontWeight:600,
                          textDecoration:"none" }}>
                View all →
              </Link>
            </div>
            ...
            {recentOrders.length === 0
              ...
              : recentOrders.slice(0, 6).map(o => {
                  const statusColor = {
                    paid:"#16a34a", approved:"#16a34a",
                    pending:"#ca8a04", failed:"#dc2626",
                  }[o.status] || "#64748b";
                  return (
                    <div key={o._id}
                      ...
                      <div style={{ textAlign:"right", flexShrink:0 }}>
                        ...
                        <span style={{ fontSize:10, fontWeight:600, color:statusColor,
                                        background:`${statusColor}14`, padding:"2px 8px",
                                        borderRadius:20 }}>
